...
//----------------------------------------------------------------------------------------------------------------------------------------------------------------------
                ...
                conn.ps.setString(1,card_no);
                conn.ps.setString(2,pin);

                // for storing in ResultSet
                ResultSet resultSet = conn.ps.executeQuery();

                //for checking that the values are there in result set or not || resultSet.next() yeh check karta hai ki "Kya resultSet (box) ke andr kam se kam ek row data hai?
                if(resultSet.next()){
                    new main_Class(pin);
                    setVisible(false);
                }else {
